...
class FloatingApp:
    ...
    def load_config(self):
        ...
        default_btns = [
            {'x': 50, 'y': 100, 'w': 100, 'h': 80, 'name': '左上', 'hover': 'w+a', 'lclick': 'j', 'rclick': 'k', 'mclick': '', 'wheelup': '', 'wheeldown': ''},
            {'x': 200, 'y': 100, 'w': 100, 'h': 80, 'name': '右上', 'hover': 'w+d', 'lclick': 'j', 'rclick': 'k', 'mclick': '', 'wheelup': '', 'wheeldown': ''},
            {'x': 125, 'y': 200, 'w': 100, 'h': 80, 'name': '后退', 'hover': 's', 'lclick': 'j', 'rclick': 'k', 'mclick': '', 'wheelup': '', 'wheeldown': ''},
        ]
        ...
        if not self.is_hidden:
            try:
                ...
                if w > 100 and h > 100:
                    ...
                    # 【修复】同步更新 saved_geometry，确保位置被记住
    ...
    def redraw_all_buttons(self):
        ...
        for idx, btn in enumerate(self.buttons_config):
            ...
            if 'mclick' not in btn: btn['mclick'] = ''
    ...
    def setup_ui(self):
        ...
        if self.current_mode == 'main':
            ...
            self.set_window_style(self.root, 'click_through')
    ...
    def to_show(self):
        ...
        if "80x80" in geo: geo = "640x480+100+100"
    ...
    def add_btn(self):
        ...
        new_btn = {'x': cw//2-40, 'y': ch//2-40, 'w': 80, 'h': 80, 'name': '按钮', 'hover': '', 'lclick': '', 'rclick': '', 'mclick': '', 'wheelup': '', 'wheeldown': ''}
    ...
    def check_active_ui_area(self, rel_x, rel_y):
        if self.is_hidden:
            return True 
        else:
            if 10 <= rel_x <= 190 and 10 <= rel_y <= 50:
                return True
            return False
    ...
    def update_loop(self):
        try:
            if self.current_mode == 'run':
                ...
                # 智能穿透逻辑
                is_on_active_ui = self.check_active_ui_area(rel_x, rel_y)
                if is_on_active_ui and not self.is_window_solid:
                    self.set_window_style(self.root, 'no_focus')
                    self.is_window_solid = True
                elif not is_on_active_ui and self.is_window_solid:
                    self.set_window_style(self.root, 'click_through')
                    self.is_window_solid = False
                ...
                middle_is_down = (user32.GetAsyncKeyState(0x04) & 0x8000) != 0
                ...
                for idx, btn in enumerate(self.buttons_config):
                    ...
                    if in_x and in_y:
                        ...
                        if middle_is_down: target_state = 'active_middle'
                    ...
                    if btn.get('last_visual_state') != target_state:
                        ...
                        if target_state != 'normal': 
                            ...
                            if target_state in ['active_left', 'active_right', 'active_middle']:
                    ...
                    if in_x and in_y:
                        ...
                        if middle_is_down and not self.middle_was_down:
                            self.holding_btn_middle = idx
                            self.trigger(btn.get('mclick', ''), 'p')
                    else:
                        if btn['active_hover']:
                ...
                if not middle_is_down and self.middle_was_down:
                    if self.holding_btn_middle is not None:
                        btn = self.buttons_config[self.holding_btn_middle]
                        if not btn.get('deleted'): self.trigger(btn.get('mclick', ''), 'r')
                        self.holding_btn_middle = None

                self.left_was_down = left_is_down
                self.right_was_down = right_is_down
                self.middle_was_down = middle_is_down 
        except: pass
        self.root.after(1, self.update_loop)
